...
def detect_bi_v17(kline_df, fractals):
    """
    v17笔检测：考虑反向笔幅度
    """
    if len(fractals) < 2:
        ...
        if current['type'] == 'bottom':
            ...
            while j < len(fractals):
                ...
                if f['type'] == 'bottom' and f['price'] < current['price']:
                    ...
                    # 检查这个底分型后是否有有效的向上笔
                    has_up_bi = False
                    for k in range(j+1, min(j+10, len(fractals))):
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"PTA缠论线段检测 v17 | {args.date}")
    print(f"{'='*50}\n")
    ...
    # 检测笔
    bi_list = detect_bi_v17(kline_df, fractals)
    print(f"\n[笔] {len(bi_list)}条")
