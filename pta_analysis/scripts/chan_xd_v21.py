...
def detect_bi_v21(kline_df, fractals):
    """
    v21笔检测：只有创新低/新高的反向分型才破坏笔
    """
    if len(fractals) < 2:
        return []
        ...
        if current_dir == 'up':
            ...
            while j < len(fractals):
                ...
                if f['type'] == 'top':
                    ...
                    # 出现底分型，检查是否创新低
                    if f['price'] < start_fractal['price']:
                        # 创新低！上行笔应该结束
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"PTA缠论线段检测 v21 | {args.date}")
    print(f"{'='*50}\n")
    ...
    # 检测笔
    bi_list = detect_bi_v21(kline_df, fractals)
    print(f"\n[笔] {len(bi_list)}条")
