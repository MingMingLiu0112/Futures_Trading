...
def check_beichi(bi_in: Bi, bi_out: Bi, kls: List[KL],
    ...
    if method == 'macd':
        ...
        in_peak = max(abs(kls[i].macd) for i in range(idx_start, min(idx_end + 1, len(kls)))) if kls else 0
        idx_start = min(bi_out.begin_idx, bi_out.end_idx)
        idx_end = max(bi_out.begin_idx, bi_out.end_idx)
        out_peak = max(abs(kls[i].macd) for i in range(idx_start, min(idx_end + 1, len(kls)))) if kls else 0
        in_metric = in_peak
        out_metric = out_peak
    elif method == 'slope':
...
# 严格买卖点配置参数
BSPointConfig = {
    'max_bs2_retrace_rate': 1.0,    # 2买：回落幅度/突破幅度 <= 1.0（回落不能超过前一UP笔幅度）
    'min_bs3_break_ratio': 0.3,     # 3买：突破幅度/中枢区间 >= 0.3
    'bs3_price_margin': 0.002,     # 3买：回落低点相对于中枢高点的容差（0.2%）
    'beichi_threshold': 0.8,       # 背驰阈值：离开段/进入段 < 0.8
    'max_3buy_count': 3,            # 最多3个3买
    'max_3sell_count': 3,          # 最多3个3卖
}


def find_bs_points(bis: List[Bi], zss: List[ZS], kls: List[KL],
                   config: Dict[str, Any] = None) -> List[BSPoint]:
    """寻找买卖点 - 严格标准
    ...
    for i in range(1, len(bis) - 2):
        ...
        if curr_bi.dir == 'down' and next_bi.dir == 'up':
            ...
            is_beichi, ratio = check_beichi(curr_bi, next_bi, kls, method='macd')
            
            # 1买确认：创新低 + MACD背驰
            if is_beichi:
                curr_kl = kls_dict.get(next_bi.end_idx)
                if curr_kl:
                    reason_parts = [f"MACD背驰(比率={ratio:.2f})"]
                    if has_in_structure:
    ...
    # 4. 回落幅度/突破幅度 <= max_bs2_retrace_rate（不能回落太深）
    if bs_points and bs_points[0].type == '1buy':
        ...
        for i in range(first_buy.bi_idx + 1, len(bis) - 1):
            ...
            # 条件1：回落不破一买（允许0.2%误差）
            if next_low < first_buy_low * (1 - config.get('bs3_price_margin', 0.002)):
                continue
            ...
            # 条件3：回落幅度/突破幅度 <= max_bs2_retrace_rate
            # 如果突破幅度为0，跳过
            if break_amp <= 0:
                continue
            retrace_rate = retr_amp / break_amp
            if retrace_rate > config.get('max_bs2_retrace_rate', 1.0):
                continue
            ...
            if i + 2 < len(bis) and bis[i + 2].dir == 'up':
                ...
                if curr_kl:
                    bs_points.append(BSPoint(
                        ...
                        reason=f"回落不破一买(回落率={retrace_rate:.2f})"
                    ))
                    bs_idx += 1
                    break
    ...
    for i in range(len(bis) - 1):
        if break_count >= config.get('max_3buy_count', 3):
            break
        bi = bis[i]
        next_bi = bis[i + 1]
        ...
        if zs_range > 0 and break_range < zs_range * config.get('min_bs3_break_ratio', 0.3):
            continue
        
        # 条件3：回落不破中枢高点（允许0.2%误差）
        price_margin = config.get('bs3_price_margin', 0.002)
        if next_low < zs_high * (1 - price_margin):
            continue
    ...
    if any(p.type == '1sell' for p in bs_points):
        ...
        if first_sell:
            ...
            for i in range(first_sell.bi_idx + 1, len(bis) - 1):
                ...
                # 条件1：反弹不破一卖（允许0.2%误差）
                if next_high > first_sell_price * (1 + price_margin):
                    continue
                ...
                # 条件3：反弹幅度/突破幅度 <= max_bs2_retrace_rate
                if break_amp <= 0:
                    continue
                retrace_rate = retr_amp / break_amp
                if retrace_rate > config.get('max_bs2_retrace_rate', 1.0):
    ...
    price_margin = config.get('bs3_price_margin', 0.002)
    for i in range(len(bis) - 1):
        if break_count_sell >= config.get('max_3sell_count', 3):
            break
        bi = bis[i]
        next_bi = bis[i + 1]
